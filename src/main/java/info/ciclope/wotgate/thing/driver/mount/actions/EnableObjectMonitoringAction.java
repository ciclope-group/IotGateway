...
public class EnableObjectMonitoringAction {
    ...
    public EnableObjectMonitoringAction(JsonObject stateProperty, Vertx vertx) {
        this.vertx = vertx;
    ...
    public void enableObjectMonitoring(Message<JsonObject> message) {
        ThingRequest request = new ThingRequest(message.body());
        JsonObject userData;
        try {
        ...
        String monitoredObject = userData.getString("monitoredObject");
        Integer monitoringInterval = userData.getInteger("monitoringInterval");
        if (userData == null || monitoringInterval == null || monitoringInterval <= 0 || monitoredObject == null ||
                (!monitoredObject.equals(MountThing.OBJECT_SUN) && !monitoredObject.equals(MountThing.OBJECT_MOON))) {
        ...
        if (stateProperty.getBoolean("onMonitoring") &&
                stateProperty.getString("monitoredObject").equals(monitoredObject)) {
    ...
    private void completeTask(long id) {
        ...
        stateProperty.put("monitoredObject", taskStorage.get(taskId).getJsonObject("inputData").getString("monitoredObject"));
        stateProperty.put("monitoringInterval", taskStorage.get(taskId).getJsonObject("inputData").getInteger("monitoringInterval"));
        stateProperty.put("altitude", 30.0);
        stateProperty.put("azimuth", 20.4);
        stateProperty.put("ra", 3.2);
        stateProperty.put("dec", 90.0);
        timerTaskMap.remove(id);
