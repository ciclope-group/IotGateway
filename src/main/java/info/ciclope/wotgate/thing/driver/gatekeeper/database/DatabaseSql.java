...
public class DatabaseSql {
    ...
    public static final String GET_ALL_RESERVATIONS = "SELECT json_group_array(reservations) FROM (SELECT json_object('startDate', reservations.startDate, 'endDate', reservations.endDate, 'userName', users.name, 'dateCreated', reservations.dateCreated) AS reservation FROM reservations LEFT JOIN users ON users.id = reservations.user);";
    public static final String GET_ALL_RESERVATIONS_BY_DATE = "SELECT json_group_array(reservations) FROM (SELECT json_object('startDate', reservations.startDate, 'endDate', reservations.endDate, 'userName', users.name, 'dateCreated', reservations.dateCreated) AS reservation FROM reservations LEFT JOIN users ON users.id = reservations.user WHERE DATE(reservations.startDate) = DATE(?/1000, 'unixepoch'));";
    public static final String GET_ALL_RESERVATIONS_BY_DATE_AND_USER = "SELECT json_group_array(reservations) FROM (SELECT json_object('startDate', reservations.startDate, 'endDate', reservations.endDate, 'dateCreated', reservations.dateCreated) AS reservation FROM reservations LEFT JOIN users ON users.id = reservations.user WHERE DATE(reservations.startDate) = DATE(?/1000, 'unixepoch') AND users.name = ?);";
    public static final String GET_ALL_RESERVATIONS_BY_USER = "SELECT json_group_array(reservations) FROM (SELECT json_object('startDate', reservations.startDate, 'endDate', reservations.endDate, 'dateCreated', reservations.dateCreated) AS reservation FROM reservations LEFT JOIN users ON users.id = reservations.user WHERE users.name = ?);";
    public static final String ADD_RESERVATION = "INSERT INTO reservations (startDate, endDate, userName) VALUES (" + DATETIME_PARAMETER + ", " + DATETIME_PARAMETER + ", (SELECT id FROM users WHERE users.name = ?));";
    public static final String DELETE_RESERVATION_BY_DATE = "DELETE FROM reservations LEFT JOIN users ON users.id = reservations.user WHERE DATETIME(reservations.startDate) = DATETIME(?/1000, 'unixepoch') AND users.name = ?;";
