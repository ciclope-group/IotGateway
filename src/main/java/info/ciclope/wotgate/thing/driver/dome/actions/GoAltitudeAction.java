...
public class GoAltitudeAction {
    ...
    public GoAltitudeAction(JsonObject stateProperty, Vertx vertx) {
        this.vertx = vertx;
    ...
    public void goAltitude(Message<JsonObject> message) {
        ThingRequest request = new ThingRequest(message.body());
        JsonObject userData;
        try {
        ...
        if (userData == null || altitude == null) {
    ...
    private void completeTask(long id) {
        ...
        stateProperty.getJsonObject("position").put("altitude", taskStorage.get(taskId).getJsonObject("inputData").getFloat("altitude"));
        timerTaskMap.remove(id);
