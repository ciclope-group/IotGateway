...
public class OpenElementAction {
    ...
    public OpenElementAction(JsonObject stateProperty, Vertx vertx) {
        this.vertx = vertx;
    ...
    public void openElement(Message<JsonObject> message) {
        ThingRequest request = new ThingRequest(message.body());
        JsonObject userData;
        try {
        ...
        if (stateProperty.getJsonArray("openingElements").getJsonObject(0).getString("status").equals("OPEN")) {
    ...
    private void completeTask(long id) {
        ...
        stateProperty.getJsonArray("openingElements").getJsonObject(0).put("status", "OPEN");
        timerTaskMap.remove(id);
