...
public class MoveByTimeAction {
    ...
    public MoveByTimeAction(JsonObject stateProperty, Vertx vertx) {
        this.vertx = vertx;
    ...
    public void moveByTime(Message<JsonObject> message) {
        ThingRequest request = new ThingRequest(message.body());
        JsonObject userData;
        try {
        ...
        String direction = userData.getString("direction");
        Integer timer = userData.getInteger("timer");
        if (userData == null || direction == null || timer == null || timer <= 0 ||
    ...
    private void completeTask(long id) {
        ...
        // TODO : Aclarar qué hace realmente el método y aplicarlo (¿movimiento temporizado o mover a coordenadas HH:MM:SS.SSS?)
        timerTaskMap.remove(id);
