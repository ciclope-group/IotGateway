...
public class GoAzimuthAction {
    ...
    public GoAzimuthAction(JsonObject stateProperty, Vertx vertx) {
        this.vertx = vertx;
    ...
    public void goAzimuth(Message<JsonObject> message) {
        ThingRequest request = new ThingRequest(message.body());
        JsonObject userData;
        try {
    ...
    private void completeTask(long id) {
        ...
        stateProperty.getJsonObject("position").put("azimuth", taskStorage.get(taskId).getJsonObject("inputData").getFloat("azimuth"));
        timerTaskMap.remove(id);
